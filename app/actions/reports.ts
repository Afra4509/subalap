...
export async function likeReport(reportId: number | string) {
  try {
    ...
    console.error("likeReport error:", error)
    return 0
  }
}
...
export async function addComment(reportId: number | string, content: string, author = "Citizen") {
  try {
    ...
    if (isToxic(safeContent)) {
      return { error: "Komentar Anda mengandung kata-kata tidak pantas. Mohon jaga kesopanan." }
    }
    ...
    if (!user || typeof reportId !== 'string') {
      return { error: "Harus login untuk berkomentar." }
    }
    ...
    return { error: error instanceof Error ? error.message : "Gagal mengirim komentar." }
  }
}
