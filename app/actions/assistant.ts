...
export async function askAssistant(query: string, localReports: any[] = []): Promise<AssistantAnswer> {
  ...
  if (safeQuery.length < 2) throw new Error("Pertanyaan terlalu pendek.")
...
${feedData.length > 0 ? feedData : 'Belum ada laporan warga terbaru.'}`
  ...
  const timeoutId = setTimeout(() => controller.abort(), 15000); // Groq is fast, 15s is plenty
  ...
  try {
    ...
    const answer = data.choices[0]?.message?.content || "Maaf, asisten sedang mengalami gangguan dalam memproses jawaban."

    return { answer, sources }
  } catch (error) {
    clearTimeout(timeoutId);
    console.error("Assistant Error:", error)
    throw new Error("Gagal mengambil jawaban dari asisten AI. Pastikan koneksi internet stabil.")
  }
}
